...
* "EVTYPE": event names
* "FATALITIES" & "INJURIES": respectively numbers of fatalities and injuries
...
```{r data_processing_2, echoes = TRUE, warning = FALSE, message= FALSE}    
      ...
      # White space is trimmed, all text is converted to uppercase
            ...
            storms_db$EVTYPE <- trimws(storms_db$EVTYPE)   # remove white space befor abd after the text
...
Somehow, this approach combines dangerousity and frequency (in the data base) of each event. For information, we will also add
...
The most harmfull events in America is Tornado with a little bit more than 1000 fatalities over 10 years.


### Most damaging meteo events
We will define the "most damaging" by computing the _total_ damage cost (properties + crops) for each events over those 10 years.
Somehow, this approach combines damaging potential and frequency (in the data base) of each event. 
...
In total, flood seems to be, by far, the most costly meteo events in America over those 10 years. It should be noted the special traitement of flood event cost estimates as described in the the "Storm data preparation" document. The flood does not have the "no information available" and a number should _always_ be provided.